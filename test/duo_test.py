...
from src.calibration.pts import get_joint_pts_model
...
def main():
    ...
    # 3. Grid Search Loop
    for lr, h_dim, bs in product(lrs, hidden_dims, batch_sizes):
        ...
        log_event(f"Completed: LR={lr}, Hidden={h_dim}, Batch={bs} | Loss: {loss:.6f}")
        
        zd_pts = get_joint_pts_logits(model, zt_large, zt_small)
        metrics = get_metrics_dict(F.softmax(zd_pts, dim=-1), labels)
        log_event(f"Metrics for LR={lr}, Hidden={h_dim}, Batch={bs}: {metrics}")
        acc = metrics['accuracy']
        if best_acc < acc:  
            best_acc = acc
            best_hyperparams_acc = (lr, h_dim, bs)
            log_event(f"New Best Accuracy: {best_acc:.4f} with LR={lr}, Hidden={h_dim}, Batch={bs}")

        if loss < best_loss:
            best_loss = loss
            best_hyperparams_loss = (lr, h_dim, bs)
            log_event(f"New Best Loss: {best_loss:.6f} with LR={lr}, Hidden={h_dim}, Batch={bs}")
