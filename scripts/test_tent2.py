...
#     print(f"--- TTA Adaptation: {distortion_name} | Severity {severity} ---")
    
#     if distortion_name == "none" and severity == 0:
#         root_path = data_path # Assumes path to clean val images
#     else:
#         root_path = os.path.join(data_path, distortion_name, str(severity))
    
#     if not os.path.exists(root_path):
#         raise FileNotFoundError(f"Warning: Path {root_path} not found. Skipping.")
...
if __name__ == "__main__":
    ...
    # cfg = load_config("cfgs/get_metrics.yaml")
    # print("Running base model without tent...")
    ...
    #                         num_workers=8
    #                         )
    # metrics_base = get_metrics_dict(F.softmax(zl, dim=-1), yl)
    # print(f"Base Metrics: {metrics_base}")

    print("\nRunning TENT with NO Temperature Scaling...")
    logits, labels = get_tent_logits_imagenet_c(model_name, distortion_name, severity, data_path, cfg, ts=None)
    metrics_raw = get_metrics_dict(F.softmax(logits, dim=-1), labels)
    print(f"Metrics: {metrics_raw}")

    print("\nRunning TENT with Naive Temperature Scaling...")
    logits, labels = get_tent_logits_imagenet_c(model_name, distortion_name, severity, data_path, cfg, ts="naive")
    metrics_raw = get_metrics_dict(F.softmax(logits, dim=-1), labels)
    print(f"Metrics: {metrics_raw}")
